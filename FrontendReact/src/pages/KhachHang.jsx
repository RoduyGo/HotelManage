...
const KhachHang = () => {
  ...
  const handleDelete = async (maKhachHang) => {
    Modal.confirm({
      title: "Xác nhận xóa",
      content: `Bạn có chắc chắn muốn xóa Khách hàng với mã: ${maKhachHang}?`,
      okText: "Xóa",
      okType: "danger",
      cancelText: "Hủy",
      onOk: async () => {
  ...
  const handleSave = async (event) => {
    ...
    try {
      if (currentCustomer) {
        await axios
          .put(`/home/khachhangs/${currentCustomer.maKhachHang}`, customerData)
          .then(() => {
            notification.success({
              message: "Thành công",
              description: "Khách hàng đã cập nhật thành công",
            });
          });
      } else {
        await axios.post("/home/khachhangs", customerData).then(() => {
          notification.success({
            message: "Thành công",
